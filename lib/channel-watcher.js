...
const { normalizeChannelUrl, detectLiveVideo } = require("./youtube-channel");

const DEFAULT_POLL_MS = 60_000;

class ChannelWatcher extends EventEmitter {
  constructor({ id, input, name, enabled, pollIntervalMs }) {
    super();
    this.id = id || `ch_${randomUUID().slice(0, 8)}`;
    this.input = input; // raw user input
    this.canonical = normalizeChannelUrl(input);
    this.name = name || null; // friendly label, optional
