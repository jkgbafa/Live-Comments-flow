...
const { YouTubeApiSource } = require("./youtube-api-source");
const {
  normalizeChannelUrl,
  detectLiveVideo,
...
class ChannelWatcher extends EventEmitter {
  ...
  _label() {
    // Just the channel/page name. The platform is already conveyed by the
    // section heading in admin and the icon next to each chat message.
    return this.name || this.displayName || this.canonical;
  }
  ...
  _spawnSource(videoId, { forceMasterchat = false } = {}) {
    // Prefer the official Data API reader (no 429 from data-center IPs) when an
    // API key is configured. Fall back to masterchat scraping otherwise, or if
    // the API quota gets exhausted mid-broadcast.
    const apiKey = process.env.YT_API_KEY;
    let src;
    if (apiKey && !forceMasterchat) {
      src = new YouTubeApiSource(videoId, apiKey);
      src.on("fallback", (snap) => {
        console.warn(
          `[yt-api] ${this.canonical} unusable (${snap.lastError}) — falling back to masterchat`
        );
        this._tearDownSource("api source fell back");
        this._spawnSource(videoId, { forceMasterchat: true });
      });
    } else {
      src = new YouTubeSource(videoId);
    }
    this._source = src;
    src.on("message", (msg) => {
    ...
    src.on("status", (snap) => {
      ...
      // If the underlying source errored persistently, surface that — but
      // suppress YouTube's "sign in to confirm bot" challenge, which we
      // routinely retry past automatically. Showing it to the user is
      // alarmist and not actionable.
      if (snap.status === "error" || snap.status === "stopped") {
        const err = snap.lastError || "";
        if (/sign in to confirm/i.test(err) || /bot/i.test(err)) {
          this.lastError = null;
        } else {
