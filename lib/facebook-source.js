...
class FacebookSource extends EventEmitter {
  ...
  async _liveDetectLoop() {
    ...
    try {
      const url = `${GRAPH}/${this.pageId}/live_videos?fields=id,status,title,permalink_url&limit=5&access_token=${encodeURIComponent(
        this.pageToken
      )}`;
      ...
      const liveItems = (data.data || []).filter((v) => v.status === "LIVE");
